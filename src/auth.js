...
import { auth, googleProvider, emailProvider } from './firebase-config.js';
...
async function loadUserWaveforms(offset = 0, append = false) {
  ...
  try {
    ...
    if (waveforms.length === 0 && offset === 0) {
      waveformsContainer.innerHTML = '<div class="muted" style="text-align: center; padding: 20px;">No MemoryWaves yet. Create your first one above!</div>';
